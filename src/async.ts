...
import { ResultValid } from "./valid";
import { ResultBaseActions } from "./base";


export class ResultAsync<T, E> extends Promise<T> implements ResultBaseActions<T, E> {
    constructor(executor: (valid: (value: T) => void, error: (value: E) => void) => void) {
        super(executor);
    }

    async sync() {
        try {
            return new ResultValid(await this);
        } catch (e) {
