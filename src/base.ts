...
export interface ResultBase<T, E> {
    ...
    andThen<T2>(mapper: (value: T) => ResultValidClass<T2>): Result<T2, E>;
    andThen<E2>(mapper: (value: T) => ResultErrorClass<E2>): Result<T, E2>;
    andThen<T2, E2>(mapper: (value: T) => Result<T2, E2>): Result<T2, E2>;
    andThenAsync<T2>(mapper: (value: T) => Promise<ResultValidClass<T2>>): Promise<Result<T2, E>>;
     ...
     * This function can be used for control flow based on `Result` values.*/
    orElse<T2>(mapper: (error: E) => ResultValidClass<T2>): Result<T2, E>;
    orElse<E2>(mapper: (error: E) => ResultErrorClass<E2>): Result<T, E2>;
    ...
    orElseAsync<E2>(mapper: (error: E) => Promise<ResultErrorClass<E2>>): Promise<Result<T, E2>>;
...
export interface OptionalBase<T> {
     ...
     * This function can be used for control flow based on `Optional` values.*/
    // orElse<T2>(mapper: (value: T) => Optional<T2>): Optional<T2>;
    // orElse<T2>(mapper: (value: T) => Optional<T2>): Optional<T>;
    // orElseAsync<T2>(mapper: (value: T) => Promise<Optional<T2>>): Promise<Optional<T2>>;
    // orElseAsync<T2>(mapper: (value: T) => Promise<Optional<T2>>): Promise<Optional<T>>;
